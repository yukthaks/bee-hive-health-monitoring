...
IMG_INPUT = BASE_DIR / 'data' / 'images' / 'train'
LBL_INPUT = BASE_DIR / 'data' / 'labels' / 'train'

#Output
IMG_OUTPUT = BASE_DIR / 'data' / 'augmented_images'
LBL_OUTPUT = BASE_DIR / 'data' / 'augmented_labels'
...
for img_file in os.listdir(IMG_INPUT):
    ...
    img_path = IMG_INPUT / img_file
    label_path = LBL_INPUT / (Path(img_file).stem + '.txt')
    ...
    for i in range(3):
        ...
        cv2.imwrite(str(IMG_OUTPUT / aug_img_name), aug_img)
        save_yolo_labels(LBL_OUTPUT / aug_lbl_name, aug_bboxes, aug_classes)
